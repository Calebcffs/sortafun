...
- typing game - 30-second WPM sprint. Type the first word shown (it holds still, the rest slide along). Pick the top 200 or top 1000 English words; each list has its own leaderboard.
- animation studio - a tiny flipbook animator (`flipbook.html`). Draw frames by hand (pen, line, square, circle, eraser with its own size slider), onion-skin the previous frames, flip through them at 8/12/16 fps, undo with the button or ctrl+z. Not a game, no score. Frames autosave to `localStorage` (this browser only). "Post to gallery" shares it. Sound mode is planned, not built.
- animation gallery - flipbooks other people posted (`anim-gallery.html`), each looping at the fps it was drawn at. Vote (one per browser) and comment. Sorted by votes, or by newest. Backed by Firestore (`animations` + `anim_comments`), so it needs the same setup as the leaderboards.
...
- leaderboards - top daily + all-time scores for typing (top 200 and top 1000 lists), circuit, puzzle. Every row is timestamped; the very last place all-time glows gold.
